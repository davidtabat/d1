				...
				$validator = Mage::getSingleton('euvatgrouper/validation_vies');
					...
					foreach($customerIds as $customerId)
					{
						...
						if($customer->getTaxvat() != "" && is_object($customer->getDefaultBillingAddress()))
						{
							...
							$new_group_id = Mage::helper('euvatgrouper/customer')->getCustomerGroupForAccount($result->getData());
							if($old_group_id != $new_group_id)
							{
                                ...
                                //Mage::getSingleton('adminhtml/session')->addNotice(Mage::helper('euvatgrouper')->__('Did not move customer to new group: '.$new_group_id.' - worked with data: '.var_export($result->getData(),true)));
                            }
						...
						} elseif($customer->getTaxVat() == '' && is_object($customer->getDefaultBillingAddress())) {
                            $old_group_id = $customer->getGroupId();
                            $new_group_id = Mage::helper('euvatgrouper/customer')->getCustomerGroupForAccount($customer->getDefaultBillingAddress()->getData());
                            if($old_group_id != $new_group_id)
                            {
                                $old_group_name = Mage::getModel('customer/group')->load($old_group_id)->getCode();
                                $new_group_name = Mage::getModel('customer/group')->load($new_group_id)->getCode();
                                $customer->setGroupId($new_group_id);
                                Mage::getSingleton('adminhtml/session')->addNotice(Mage::helper('euvatgrouper')->__('Moved customer '.$customer->getId().' from '.$old_group_name.' to '.$new_group_name));
                            }
