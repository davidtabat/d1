...
class Infortis_Ultimo_Helper_Labels extends Mage_Core_Helper_Abstract
{
	...
	public function getLabels($product)
	{
		...
		if ($product->getData('bestseller')) {
			$html .= '<span class="sticker-wrapper top-left"><span class="sticker bestseller">' . $this->__('Bestseller') . '</span></span>';	
		}

		if ($product->getData('kauftipp')) {
			$html .= '<span class="sticker-wrapper top-left"><span class="sticker kauftipp">' . $this->__('Kauftipp') . '</span></span>';	
		}
