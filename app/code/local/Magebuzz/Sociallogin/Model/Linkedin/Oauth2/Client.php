 ...
 */
class Magebuzz_Sociallogin_Model_Linkedin_Oauth2_Client
	{
		const REDIRECT_URI_ROUTE = 'sociallogin/index/connect';

		const XML_PATH_ENABLED = 'sociallogin/linkedin/enabled';
		const XML_PATH_CLIENT_ID = 'sociallogin/linkedin/client_id';
		const XML_PATH_CLIENT_SECRET = 'sociallogin/linkedin/client_secret';
		...
		protected function _httpRequest($url, $method = 'GET', $params = array())
		{
			...
			Mage::helper('sociallogin')->log($response->getStatus().' - '. $response->getBody());
			...
			if($response->isError()) {
				...
				if(($status == 400 || $status == 401)) {
					...
					throw new Magebuzz_Sociallogin_Model_Linkedin_Oauth2_Exception($message);
				} else {
					$message = sprintf(
						Mage::helper('sociallogin')
