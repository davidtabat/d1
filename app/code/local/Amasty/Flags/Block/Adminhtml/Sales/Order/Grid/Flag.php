...
class Amasty_Flags_Block_Adminhtml_Sales_Order_Grid_Flag extends Mage_Adminhtml_Block_Template
{
    ...
    {
        if(get_class($this->getOrder()) == 'Mage_Sales_Model_Order_Shipment') {
            $orderId        = $this->getOrder()->getOrderId();
        } else {
            $orderId        = $this->getOrder()->getId();
        }

        $orderFlag = Mage::getModel('amflags/order_flag')->loadByColumnIdAndOrderId($orderId, str_replace('priority', '', $this->getCurrentColumn()->getId()));
