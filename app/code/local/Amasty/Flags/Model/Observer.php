...
{
    protected $_controllerNames = array('sales_', 'orderspro_','shipment_');
    ...
    {
        /* Custom Commented by Ahmed */
            //return in_array($page, array('adminhtml_sales_order','sales_order', 'orderspro_order'));
        /* End Custom Commented by Ahmed */

        /* Custom Code by Ahmed */
            return in_array($page, array('adminhtml_sales_order','sales_shipment', 'sales_order', 'orderspro_order'));
        /* End Custom Code by Ahmed */
    }
                ...
                if($place == 'order') {
                    ...
                    if ($column->getApplyFlag()
                        ...
                        $collection->getSelect()
                            ->joinLeft(
                                array('f2o'.$column->getEntityId() => Mage::getModel('amflags/order_flag')->getResource()->getMainTable()),
                                'f2o'.$column->getEntityId().'.order_id = ' . $alias . '.order_id ' .
                                'AND f2o'.$column->getEntityId().'.column_id = '.$column->getEntityId(),
                                array())
                            ->joinLeft(
                                array('f'.$column->getEntityId() => Mage::getModel('amflags/flag')->getResource()->getMainTable()),
    ...
    public function onCoreCollectionAbstractLoadBefore($observer)
    {
        ...
        if (version_compare(Mage::getVersion(), '1.5', '<')) {
            ...
            // $shipmentCollectionClass =Mage::getConfig()->getResourceModelClassName('sales/order_collection');
            // echo '1.5'. get_class($orderCollectionClass); exit();
        } else {
            $orderCollectionClass = Mage::getConfig()->getResourceModelClassName('sales/order_grid_collection');
            $shipmentCollectionClass = Mage::getConfig()->getResourceModelClassName('sales/shipment_grid_collection');
        ...
        }elseif ($collection instanceof Mage_Sales_Model_Resource_Order_Shipment_Grid_Collection
            || $shipmentCollectionClass == get_class($collection)) {
            if (self::$collectionModified && !Mage::helper('core')->isModuleEnabled('BL_CustomGrid'))
                return;
            else
                self::$collectionModified = true;
            
            $this->_prepareCollection($collection,'shipment');
        }
    }
    ...
    public function onCoreLayoutBlockCreateAfter($observer)
    {
        ...
        /* Custom Code by Ahmed */
            $blockClass2 = Mage::getConfig()->getBlockClassName('adminhtml/sales_shipment_grid');
            if ($blockClass2 == get_class($block)) {
                $columnCollection = Mage::getModel('amflags/column')->getCollection();
                $columnCollection->getSelect()->order('pos ASC');
                if ($columnCollection->getSize() > 0) {
                    $this->_prepareColumns($block, $columnCollection, in_array(Mage::app()->getRequest()->getActionName(), $this->_exportActions),'shipment','created_at');
                }
            }
        /* Custom Code by Ahmed End */
