...
class Infomodus_Upslabel_Block_Adminhtml_Lists_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    public function callback_print($value, $row, $column, $isExport)
    {
        ...
        if ($row->getTypePrint() == "GIF") {
            $Pdf = '<a href="' . $this->getUrl('adminhtml/upslabel_pdflabels/onepdf/order_id/' . $row->getOrderId() . '/shipment_id/' . $row->getShipmentId() . '/type/' . $row->getType()) . '" target="_blank">PDF</a>';
            $Image = '<a href="' . $this->getUrl('adminhtml/upslabel_upslabel/print/imname/' . 'label' . $row->getTrackingnumber() . '.gif') . '" target="_blank">Image</a>';
        }
        else {
            echo '<a href="' . $this->getUrl('adminhtml/upslabel_upslabel/autoprint/order_id/'.$row->getOrderId().'/shipment_id/'.$row->getShipmentId.'/type/'.$row->getType) . '" target="_blank">' . Mage::helper('adminhtml')->__('Print Label') . '</a>';
        }
