...
<Locale>
        ...
        if ($this->international_invoice == 1 && count($this->international_products) > 0) {
            ...
            foreach ($this->international_products AS $interproduct) {
                $data .= "<Product>
            <Description>" . $interproduct['description'] . "</Description>
            <OriginCountryCode>" . $interproduct['country_code'] . "</OriginCountryCode>
            <Unit>
            <Number>" . $interproduct['qty'] . "</Number>
            <Value>" . $interproduct['amount'] . "</Value>
            <UnitOfMeasurement>
            <Code>" . $interproduct['unit_of_measurement'] . "</Code>";
                if(isset($interproduct['unit_of_measurementdesc']) && strlen($interproduct['unit_of_measurementdesc'])>0){
                $data .= "<Description>" . $interproduct['unit_of_measurementdesc'] . "</Description>";
                }
                $data .= "</UnitOfMeasurement>
            </Unit>
            <CommodityCode>" . $interproduct['commoditycode'] . "</CommodityCode>
            <PartNumber>" . $interproduct['partnumber'] . "</PartNumber>
            </Product>";
            }
...
<Height>" . Mage::getStoreConfig('upslabel/dimansion_' . $pv['dimansion_id'] . '/height') . "</Height>";
      ...
      <PackageServiceOptions>";
            if ($pv['insuredmonetaryvalue'] > 0) {
                $data .= "<InsuredValue>
                <CurrencyCode>" . (isset($pv['currencycode'])?$pv['currencycode']:'') . "</CurrencyCode>
...
<RatingServiceSelectionRequest xml:lang=\"en-US\">
  ...
  <Shipment>";
   ...
   <RateInformation>
      <NegotiatedRatesIndicator/>
    </RateInformation><TaxInformationIndicator/>";
        }
        $data .= "<Shipper>";
