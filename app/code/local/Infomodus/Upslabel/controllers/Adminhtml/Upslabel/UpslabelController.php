...
class Infomodus_Upslabel_Adminhtml_Upslabel_UpslabelController extends Mage_Adminhtml_Controller_Action
{
    ...
    private function cutStringUpsAddress($str, $maxNumberCaracters = 30) {
        $strg = str_replace(array('strasse', 'straße'), 'str.', trim($str));
        $numbersExtracted = array();
        preg_match_all('!\d+!', $strg, $numbersExtracted);
        $numbers = implode(', ', $numbersExtracted[0]);
        $words = preg_replace('/[0-9]+/', '', $strg);
        $newAddress = $numbers . ", " . $words;
        return substr(trim($newAddress), 0, $maxNumberCaracters);
    }
    ...
    public function showlabelAction()
    {
        ...
        $maxNumberCaracters = 30;
        $params['shiptocompanyname'] = substr(trim($params['shiptocompanyname']), 0, $maxNumberCaracters);
        $params['shiptoattentionname'] = substr(trim($params['shiptoattentionname']), 0, $maxNumberCaracters);
        
        if(strlen($params['shiptoaddressline1']) > $maxNumberCaracters) {
            $params['shiptoaddressline1'] = $this->cutStringUpsAddress($params['shiptoaddressline1']);
        }
        
        if(strlen($params['shiptoaddressline2']) > $maxNumberCaracters) {
            $params['shiptoaddressline2'] = $this->cutStringUpsAddress($params['shiptoaddressline2']);
        }
            ...
            if ($shipment_id == 0 && $type == "shipment") {
                if ($this->imOrder->canShip()) {
                    ...
                    if(Mage::getStoreConfig('upslabel/additional_settings/orderstatuses') != ''){
                        $this->imOrder->setStatus(Mage::getStoreConfig('upslabel/additional_settings/orderstatuses'), true)->save();
                    }
                } else {
    ...
    function setParams($lbl, $params, $packages )
    {
        ...
        $lbl->shipperStateProvinceCode = Infomodus_Upslabel_Helper_Help::escapeXML($configOptions->getProvinceCode(Mage::getStoreConfig('upslabel/address_' . $params['shipper_no'] . '/stateprovincecode'), $params['shiptocountrycode']));
        ...
        $lbl->shipfromStateProvinceCode = Infomodus_Upslabel_Helper_Help::escapeXML($configOptions->getProvinceCode(Mage::getStoreConfig('upslabel/address_' . $params['shipfrom_no'] . '/stateprovincecode'), $params['shiptocountrycode']));
    ...
    function intermediatehandy($order_id, $type, $shipment_id = NULL)
    {
        ...
        foreach ($shipmentAllItems AS $item) {
            ...
            $this->defConfParams['international_products'][] = array(
                ...
                'commoditycode' => $productOrigin[Mage::getStoreConfig('upslabel/paperless/international_commodity_id')],
                'partnumber' => $pi,
            );
            $pi++;
        }
        ...
        if ($this->totalWeight <= 0) {
            $this->totalWeight = Mage::getStoreConfig('upslabel/weightdimension/defweigth');
        }
