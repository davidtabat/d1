...
class Mirasvit_Helpdesk_Helper_String extends Varien_Object
{
     ...
     * @param string $acceptForeignTickets
     *
     * @return bool|string
     */
    public function getTicketCodeFromSubject($subject, $acceptForeignTickets)
    {
        ...
        //aw tickets
        if (($acceptForeignTickets == Mirasvit_Helpdesk_Model_Config::ACCEPT_FOREIGN_TICKETS_AW) &&
                $subject && preg_match('[[#][A-Z]{1,3}-[0-9]{1,5}]', $subject, $regs)) {
            return ltrim($regs[0], '#');
        }

        //mw tickets
        if (($acceptForeignTickets == Mirasvit_Helpdesk_Model_Config::ACCEPT_FOREIGN_TICKETS_MW) &&
     ...
     */
    public function convertToHtml($plainText)
    {
        $html = $plainText;
        $html = preg_replace(
            array(
