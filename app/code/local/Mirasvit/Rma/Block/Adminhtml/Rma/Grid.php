...
class Mirasvit_Rma_Block_Adminhtml_Rma_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    public function _itemsFilterCallback($collection, $column)
    {
        ...
        $collection->getSelect()
            ->join(array('item' => $collection->getTable('rma/item')), 'item.rma_id = main_table.rma_id', array('main_table.item_name' => 'item.name'))
            ->join(array('item_cond' => $collection->getTable('rma/condition')), 'item_cond.condition_id = item.condition_id', array('main_table.item_condition' => 'item_cond.name'))
            ->join(array('item_reason' => $collection->getTable('rma/reason')), 'item_reason.reason_id = item.reason_id', array('main_table.item_reason' => 'item_reason.name'))
            ->join(array('item_resolution' => $collection->getTable('rma/resolution')), 'item_resolution.resolution_id = item.resolution_id', array('main_table.item_resolution' => 'item_resolution.name'))
            ;

        $collection->addFieldToFilter(
            array('item.name', 'item_cond.name', 'item_reason.name', 'item_resolution.name'),
            array(
                $filterArray,
                $filterArray,
                $filterArray,
