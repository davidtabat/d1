...
class IWD_OrderManager_Block_Adminhtml_Cataloginventory_Product_Grid_Renderer_Qty extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{
    ...
    public function render(Varien_Object $row)
    {
        ...
        return sprintf(
            "<input title='Is in stock?' type='checkbox' %s value='1' class='is-in-stock' name='stock[%s][%s][is_in_stock]'/>
<input type='text' class='input-text %s' %s name='stock[%s][%s][qty]' value='%s' data-product-id='%s' title='Qty'>",
            $isInStock, $productId, $stockId, $class, $id, $productId, $stockId, $qty, $productId
        );
    }
}